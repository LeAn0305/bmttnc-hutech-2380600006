    ...
    def call_api_encrypt(self):
        ...
        if not is_english_letters_only(plain_text):
            self.show_message(
                QMessageBox.Warning,
                "Lỗi nhập liệu",
                "PlainText của Playfair chỉ được chứa chữ cái A-Z hoặc a-z, không được có khoảng trắng, số, dấu tiếng Việt hoặc ký tự đặc biệt nha.",
            )
            return
        ...
        if not is_english_letters_only(key_input):
            self.show_message(
                QMessageBox.Warning,
                "Lỗi nhập liệu",
                "Key của Playfair chỉ được chứa chữ cái A-Z hoặc a-z, không được có khoảng trắng, số, dấu tiếng Việt hoặc ký tự đặc biệt nha.",
            )
            return
    ...
    def call_api_decrypt(self):
        ...
        if not is_english_letters_only(cipher_text):
            self.show_message(
                QMessageBox.Warning,
                "Lỗi nhập liệu",
                "CipherText của Playfair chỉ được chứa chữ cái A-Z hoặc a-z, không được có khoảng trắng, số, dấu tiếng Việt hoặc ký tự đặc biệt nha.",
            )
            return
